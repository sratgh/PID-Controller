...
int main() {
  ...
  steer_pid.Init(0.1, 0.0001, 1.5);
  speed_pid.Init(0.1, 0.0001, 0.0);

  h.onMessage([&steer_pid, &speed_pid, &desired_speed](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
           ...
           */
           ...
           // Keep throttle value within boundaries
